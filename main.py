...
class PhraseVault(ThemedTk):
    def __init__(self):
        ...
        self.listbox.bind('<Return>', self.copy_to_clipboard)
...
class AddEditPhraseWindow(tk.Toplevel):
    def __init__(self, parent, entry_id=None):
        ...
        self.phrase_entry.grid(row=0, column=1, padx=5, pady=5, sticky="nsew")

        ttk.Label(self, text="Expanded Text:").grid(
            row=1, column=0, padx=5, pady=5, sticky="w")
        self.expanded_text_entry = tk.Text(
            self, wrap=tk.WORD, height=5, width=20)
...
if __name__ == "__main__":
    ...
    app.tray_icon = pystray.Icon("PhraseVault", tray_image, "PhraseVault", menu=pystray.Menu(
        pystray.MenuItem('Toggle', toggle_main_window, default=True),
        pystray.MenuItem('Quit', on_quit)))

    tray_icon_thread = threading.Thread(target=app.tray_icon.run)
    tray_icon_thread.start()

    with keyboard.GlobalHotKeys({'<ctrl>+#': toggle_main_window, '<ctrl>+\\': toggle_main_window}) as hotkey_listener:
        app.mainloop()
