...
describe('parsePlaceholders', () => {
  ...
  it('parses placeholder with options', () => {
    ...
    expect(result[0].type).toBe('date');
    expect(result[0].options.default).toBe('long');
  });
...
describe('date formatting', () => {
  ...
  it('formats weekday', () => {
    // Test with explicit locale to avoid system locale dependency
    const result = resolveAutoPlaceholders('{{weekday@en-US}}', '');
  ...
  it('formats month', () => {
    // Test with explicit locale to avoid system locale dependency
    const result = resolveAutoPlaceholders('{{month@en-US}}', '');
